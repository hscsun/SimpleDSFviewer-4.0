 ...
 if mb==0
     ...
     if getsmooth==0    
         if isnormalise==0
            ...
            lig=[lig,['(',num2str(ceil(y/12)),':',num2str(y-floor(y/12)*12),') =>Tms:',num2str(te(locvs2(find(pkvs2==max(pkvs2)))))]];
     ...
     elseif getsmooth==2
        ...
        lig=[lig,['(',num2str(ceil(y/12)),':',num2str(y-floor(y/12)*12),') =>Tms:',num2str(te(locvs))]];
     elseif getsmooth==1
     ...
     if y==97
         ...
         if getsmooth==0 
             if isnormalise==0
                ...
                lig=[lig,['(',num2str(ceil(y/12)),':',num2str(y-floor(y/12)*12),') =>Tm:',num2str(te(locvs2(find(pkvs2==max(pkvs2)))))]];
             elseif isnormalise==1
                if max(FDs(:,y))<=0
                kkk=-1;
                else
        ...
        elseif getsmooth==2
            ...
            lig=[lig,['(',num2str(ceil(y/12)),':',num2str(y-floor(y/12)*12),') =>Tm:',num2str(te(locvs))]];
        elseif getsmooth==1
            if max(FDs(:,y))<=0
                kkk=-1;
            else
...
global te sorts FDs norsorts norFDs smsorts smFDs biggest maxsmresult  DT50
[Rname Rpath] = uiputfile({'*.xlsx'},'save as');
str = [Rpath Rname];
...
Tmresult(:,3)=TmDT50;
 ...
 Tmresult(:,2)=TmSmoothFD; 
 Tmresult(:,3)=TmDT50; 
 xlswrite(str,{'Well','Tm (FD)','Tm (smoothedFD)','Tm (DT50)'},'Tm','A1');
 xlswrite(str,names,'Tm','A2');
 xlswrite(str,Tmresult,'Tm','B2');

 rdata(:,1)=te;
 rdata(:,2:98)=sorts;
 rdata(:,101)=te;
 rdata(:,102:198)=FDs;
xlswrite(str,{'Temperature','A1','A2','A3','A4','A5','A6','A7','A8','A9','A10','A11','A12',...
                             ...
                             'H1','H2','H3','H4','H5','H6','H7','H8','H9','H10','H11','H12',...
                             ''},'MCs and FDs','A1');
 xlswrite(str,rdata,'MCs and FDs','A2');
  ...
  xlswrite(str,{'Temperature','A1','A2','A3','A4','A5','A6','A7','A8','A9','A10','A11','A12',...
                             ...
                             ''},'nMC and nFD','A1');
 nordata(:,1)=te;
 nordata(:,2:98)=norsorts;
 nordata(:,101)=te;
 nordata(:,102:198)=norFDs;
 xlswrite(str,nordata,'nMC and nFD','A2');
 ...
 smdata(:,2:98)=smsorts;
 smdata(:,101)=te;
 smdata(:,102:198)=smFDs;
 xlswrite(str,smdata,'smMC and smFD','A2');
...
global te sorts   colorpicker smsorts cvname cvpath DT50 MDT50
 ...
 if mb==0
     ...
     if isnormalise==0       
        plot(te,sorts(:,y),'color',co,'LineWidth',2);   
        lig=[lig,['(',num2str(ceil(y/12)),':',num2str(y-floor(y/12)*12),') =>Tm: ',num2str(DT50(y))]];
     elseif isnormalise==1
        debg2=smsorts(:,y)-min(smsorts(:,y));
        plot(te,(debg2/max(debg2)),'color',co,'LineWidth',2);
        lig=[lig,['(',num2str(ceil(y/12)),':',num2str(y-floor(y/12)*12),') =>Tm: ',num2str(DT50(y))]];
...
xlswrite(str,['Temperature',selename],'MCs','A1');
xlswrite(str,SeleMCs,'MCs','A2');

 seledata2=FDs(:,sel);
 SeleFDs(:,1)=te;
 SeleFDs(:,2:(length(sel)+1))=seledata2;

 xlswrite(str,['Temperature',selename],'FDs','A1');
 xlswrite(str,SeleFDs,'FDs','A2'); 
 
 seledata3=norsorts(:,sel);
 SeleNorMCs(:,1)=te;
 SeleNorMCs(:,2:(length(sel)+1))=seledata3;
 
 xlswrite(str,['Temperature',selename],'nMCs','A1');
 xlswrite(str,SeleNorMCs,'nMCs','A2'); 
...
xlswrite(str,['Temperature',selename],'nFDs','A1');
xlswrite(str,SeleNorFDs,'nFDs','A2');

 seledata5=smsorts(:,sel);
 SelesmMCs(:,1)=te;
 SelesmMCs(:,2:(length(sel)+1))=seledata5;
 xlswrite(str,['Temperature',selename],'smMCs','A1');
 xlswrite(str,SelesmMCs,'smMCs','A2'); 

 seledata6=smFDs(:,sel);
 SelesmFDs(:,1)=te;
 SelesmFDs(:,2:(length(sel)+1))=seledata6;

 xlswrite(str,['Temperature',selename],'smFDs','A1');
 xlswrite(str,SelesmFDs,'smFDs','A2');
